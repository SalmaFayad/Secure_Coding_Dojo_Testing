...
var hideViewSol = fs.readFileSync(
  path.join(__dirname, 'static/hide_view_solutions.html'),
  'utf8');
...
app.get('/api/hide_view_solutions', (req, res) => {
  // console.log(req.user)
  if(req.user.accountId ==='Local_admin'){
    //let updatedHideViewSol = auth.addCsrfToken(req, hideViewSol);
    //res.send(updatedHideViewSol);
    res.status(200).send();
...
//miss Authorization error
app.get('/api/solutionoption', (req, res) => {
  // console.log(req.user)
  if(req.user.accountId ==='Local_admin'){
...
function(req, res) {
  ...
  if(req.body.username ==="admin"){
    res.redirect('/main#!/hide_view_solutions');}
    else
    {
      console.log(req.body.username);
      res.redirect('/main');
...
app.post("/hiddedchallenges", function(req, res) {
  var hiddenChallenges = req.body.data;
  fs.writeFile('hiddenchallenges.json', JSON.stringify(hiddenChallenges), function(err) {
    if (err) {
      console.error(err);
      res.status(500).send('Error writing to file');
    } else {
      // res.send('Hidden challenges saved');
      res.status(200).send()
    }
  });
});
...
app.post("/gethiddenchallenges", function(req, res) {
  var list = JSON.parse(fs.readFileSync('hiddenchallenges.json', 'utf8'));
